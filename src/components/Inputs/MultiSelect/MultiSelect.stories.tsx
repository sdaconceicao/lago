...
export const WithSelectAll: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Setting allowsSelectAll adds "select all" and "select none" controls to the dropdown as a toolbar of real buttons pinned above the options — outside the listbox, so they never match a typed filter or show up as options to a screen reader. ArrowUp from the search input (while focus is not deeper in the list) moves to them; ArrowDown from a control returns to the input. Tab reaches them as well. Enter runs whichever is focused. Pressing a control only changes the options below it, and the menu stays open exactly as it does when a single option is toggled. Both act on the options currently on offer, so with a filter typed they apply to the matches shown and leave the rest of the selection alone; disabled options are never selected. Pass selectAllLabel and selectNoneLabel to relabel them.',
      },
    },
  },
};
